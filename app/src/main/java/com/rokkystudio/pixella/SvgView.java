...
{
    private BitmapDrawable mCachedDrawable = null;
    private PictureDrawable mOriginalDrawable = null;
    ...
    */
    ...
    private float mDocumentWidth;
    private float mDocumentHeight;
    ...
    private void sharedConstructing(Context context) {
        mLinkPaint.setColor(0x337777BB);
    ...
    public void setSVG(SVG svg)
    {
        ...
        RectF docRect = mSVG.getDocumentViewBox();
        //docRect.right = docRect.right + 50;
        //mSVG.setDocumentViewBox(docRect.left, docRect.top, docRect.right, docRect.bottom);

        mDocumentWidth = docRect.right - docRect.left;
        mDocumentHeight = docRect.bottom - docRect.top;

        renderOptions.viewPort(0, 0, mDocumentWidth, mDocumentHeight);
        // Стиль ссылок в документе / Отсутсвующие стили в русских документах
        renderOptions.css("a {fill:#226688; font-weight:bold;} text.t2 {font-size:42;} text.t3 {font-size:12;");

        // Максимальный масштаб подбирается пропорционально размеру изображения
        setMaxZoom(Math.max(mDocumentWidth, mDocumentHeight) / 400f);
        Picture picture = svg.renderToPicture(renderOptions);
        mOriginalDrawable = new PictureDrawable(picture);
        mOriginalDrawable.setBounds(0, 0, (int) mDocumentWidth, (int) mDocumentHeight);

        // Bitmap не может быть больше 2048, вычисляем размеры пропорциональные размерам SVG
        int bitmapWidth = 2048;
        int bitmapHeight = 2048;
        if (mDocumentWidth > mDocumentHeight) {
            bitmapWidth = (int) (mDocumentWidth / mDocumentHeight * 2048);
        } else {
            bitmapHeight = (int) (mDocumentHeight / mDocumentWidth * 2048);
        }
        // Отрисовка документа в Bitmap в соответствии с пропорционально уменьшенными размерами
        renderOptions.viewPort(0, 0, bitmapWidth, bitmapHeight);

        Bitmap bitmap = Bitmap.createBitmap(bitmapWidth, bitmapHeight, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        canvas.drawColor(Color.WHITE);
        mSVG.renderToCanvas(canvas, renderOptions);
        mCachedDrawable = new BitmapDrawable(getResources(), bitmap);
        // Чтобы при последующей отрисовки SVG и Bitmap они накладывались друг на друга без сдвига, используем размеры SVG
        mCachedDrawable.setBounds(0, 0, (int) mDocumentWidth, (int) mDocumentHeight);
        requestLayout();
        postInvalidate();
    }
    ...
    public Bitmap getBitmap() {
        ...
        if (mCachedDrawable != null) {
            bitmap = mCachedDrawable.getBitmap();
        }
        return bitmap;
    }
    ...
    public RectF getZoomedRect() {
        ...
        float w = mOriginalDrawable.getIntrinsicWidth();
        float h = mOriginalDrawable.getIntrinsicHeight();
     ...
     * and ScaleType.
     */
    public void setZoom(SvgView img) {
    ...
    public PointF getScrollPosition() {
        ...
        int drawableWidth = mOriginalDrawable.getIntrinsicWidth();
        int drawableHeight = mOriginalDrawable.getIntrinsicHeight();
    ...
    {
        if (mOriginalDrawable == null) return;
        if (mOriginalDrawable.getIntrinsicWidth() == 0) return;
        if (mOriginalDrawable.getIntrinsicHeight() == 0) return;
        if (mCachedDrawable == null) return;
        if (mCachedDrawable.getIntrinsicWidth() == 0) return;
        if (mCachedDrawable.getIntrinsicHeight() == 0) return;
    ...
    {
        mMatrix.getValues(m);
        float origW = mOriginalDrawable.getIntrinsicWidth();
        float origH = mOriginalDrawable.getIntrinsicHeight();
        float transX = m[Matrix.MTRANS_X];
        float transY = m[Matrix.MTRANS_Y];
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (mOriginalDrawable == null || mCachedDrawable == null) return;
        ...
        canvas.save();
        canvas.clipRect(0, 0, mOriginalDrawable.getIntrinsicWidth(), mOriginalDrawable.getIntrinsicHeight());
        ...
        if (mState == State.NONE) {
            ...
            mCachedDrawable.draw(canvas);
        }

        for (SVG.A link : mSVG.getLinks()) {
            if (link.bound != null) {
                canvas.drawRect(
                    link.bound.left - mSVG.getDocumentViewBox().left,
                    link.bound.top - mSVG.getDocumentViewBox().top,
                    link.bound.right - mSVG.getDocumentViewBox().left,
                    link.bound.bottom - mSVG.getDocumentViewBox().top, mLinkPaint);
            }
        }
    ...
    {
        if (mOriginalDrawable == null || mOriginalDrawable.getIntrinsicWidth() == 0 || mOriginalDrawable.getIntrinsicHeight() == 0) {
        ...
        viewWidth = setViewSize(widthMode, widthSize, mOriginalDrawable.getIntrinsicWidth());
        viewHeight = setViewSize(heightMode, heightSize, mOriginalDrawable.getIntrinsicHeight());
    ...
    public String getLinkFromPoint(PointF point)
    {
        String href = null;
        point.x += mSVG.getDocumentViewBox().left;
        point.y += mSVG.getDocumentViewBox().top;

        float minDist = Float.MAX_VALUE;
        for (SVG.A link : mSVG.getLinks())
        {
            RectF rect = new RectF(link.bound);
            rect.inset(-3 * rect.width(), -2 * rect.height());
            if (rect.contains(point.x, point.y))
            {
                float dist = rect.centerX() + rect.centerY() - point.x - point.y;
                if (dist < minDist) {
                    minDist = dist;
                    href = link.href;
                }
